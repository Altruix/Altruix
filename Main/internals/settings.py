...
from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton, CallbackQuery, KeyboardButton, ReplyKeyboardMarkup, ReplyKeyboardRemove
...
def get_sessions_buttons(page=1) -> Tuple[List[InlineKeyboardButton], bool, int]:
    per_page = 9
    buttons = [
        InlineKeyboardButton(str(i.myself.first_name), f'session_info_{index}_{page}') for index, i in enumerate(Altruix.clients)
...
async def sessions_menu_cb_handler(c: Client, cb: CallbackQuery):
    ...
    last_col.append(InlineKeyboardButton(f"🔙 [{page}/{total_pages or 1}]", "settings_menu"))
    if has_next:
        last_col.append(InlineKeyboardButton("Next", f"sessions_list_{page + 1}"))
    await cb.message.edit(
        text="Sessions",
        reply_markup=InlineKeyboardMarkup(buttons + [last_col])
    )
...
async def unlink_session_cb_handler(c: Client, cb: CallbackQuery):
    temp = await cb.message.reply("Are you sure you want to unlink this session?", quote=True, reply_markup=ReplyKeyboardMarkup(
